...
-- TODO: include the node text in the node
data Node = Node
  { nodeType :: String,
    nodeSymbol :: Symbol,
    nodeRange :: Range,
...
convertNode language node@Raw.Node {nodeType, nodeSymbol, nodeEndPoint, nodeEndByte, nodeFieldName, nodeIsNamed, nodeIsExtra} = do
  ...
  nodeFieldName <- peekCString nodeFieldName
  ...
  pure Node {nodeType, nodeSymbol, nodeRange, nodeFieldName, nodeIsNamed = convertCBool nodeIsNamed, nodeIsExtra = convertCBool nodeIsExtra}
...
parse language source = unsafePerformIO $ do
  ...
  tree <- convertTree language tree
  pure tree
